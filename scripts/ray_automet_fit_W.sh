...
#   usage: CKPT_PFX=ckpts CKPT_DIR=lewm_k1_cube_s3072 [TAG=...] ray_automet_fit_W.sh <task>
...
python scripts/automet_fit_W.py "$TASK" --ckpt "$CKPT_DIR/weights_epoch_10.pt" ${TAG:+--out-tag "$TAG"} 2>&1 | tee "$SSD/automet_$TASK.log"
gcloud storage cp eval_results/automet_*.pt eval_results/automet_*.json "$BUCKET/eval/" || true
gcloud storage cp "$SSD/automet_$TASK.log" "$BUCKET/eval/" || true
