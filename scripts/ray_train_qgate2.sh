#!/usr/bin/env bash
# Stage-2 Auto-SCALE trainer (train_qgate2.py): identical staging to the qnative
# cube trainer; additionally fetches the chosen g* JSON when QGATE_GCS is set and
# exports QGATE_JSON for the gated arm (unset -> SCALE-All control).
# NATIVE-FULL q-only trainer (train_qgate2.py registers extra variants): replica launcher + reacher.lance staging (the q-input
...
case "$TASK" in
  ...
  tworoom)
    [ -d "$DS/tworoom.lance" ] || \
      gcloud storage rsync -r "$BUCKET/datasets/tworoom.lance" "$DS/tworoom.lance" ;;
  pointmaze)
  ...
  cube_pip)
    mkdir -p "$DS/ogbench"
    [ -d "$DS/ogbench/cube_pip.lance" ] || \
      gcloud storage rsync -r "$BUCKET/datasets/ogbench/cube_pip.lance" "$DS/ogbench/cube_pip.lance" ;;
  cube_triple)
    mkdir -p "$DS/ogbench"
...
if [ -n "${QGATE_GCS:-}" ]; then
  gcloud storage cp "$QGATE_GCS" "$SSD/g_star.json"
  export QGATE_JSON="$SSD/g_star.json"
  echo "[qgate2] gate file: $QGATE_GCS"
fi
RUN=$(python train_qgate2.py --cfg job --resolve "$@" 2>/dev/null \
...
python train_qgate2.py "$@" 2>&1 | tee "$LOG"
