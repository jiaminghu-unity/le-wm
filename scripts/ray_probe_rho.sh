...
python scripts/probe_rho.py "$TASK" 2>&1 | tee "$SSD/rho_$TASK.log"

gcloud storage cp "eval_results/rho_$TASK.json" "$BUCKET/eval/"
gcloud storage cp "$SSD/rho_$TASK.log" "$BUCKET/eval/"
echo "RHO $TASK DONE"
