...
DST_GCS="gs://prism-training-us/le-wm/qcheck/"
...
python3 -m pip install -q --user h5py hdf5plugin numpy torch --extra-index-url https://download.pytorch.org/whl/cpu 2>&1 | tail -2
python3 scripts/check_cube_q.py "$OUT_H5" "$SSD/qcheck"

gcloud storage cp "$SSD/qcheck/"* "$DST_GCS"
