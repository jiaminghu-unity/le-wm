...
SEEDS="104 105 106"
LOG=/workspace/le-wm/eval_results/final/cube_sweep.log
mkdir -p "$(dirname "$LOG")"

JOBS=(
 # Cube round 2: the three extra episode seeds, plus the combo arm this time.
 # Combo was left out of the pusht/reacher batch on request; it is included here
 # because the two combo checkpoints are the only ones that exist (pusht c6, cube k6)
 # and the additivity question needs the same seed count as its own baseline.
...
PY
  ...
  if [ "$complete" -ge "$total" ]; then log "ALL CUBE-SWEEP CELLS COMPLETE"; exit 0; fi
