#!/usr/bin/env bash
# PiP campaign (2026-09-06): visible structured distractor. cube_pip.lance =
# cube obs with paired scene frame in bottom-left 48px + alien_q column.
# Gates: REUSED from ALIEN (identical pairing; Stage-1 never sees images).
# Arms (seed 3072):
#   pipbase  vanilla LeWM on PiP obs (overlay-harm control)
#   pipq     SCALE 48-d concat q, ungated
#   pipg05   gated, ALIEN InfoNCE gate lam 0.05
#   pipg10   gated, ALIEN InfoNCE gate lam 0.1
# Evals: cem/icem/mppi x 6 seeds each -> final_eval/final_cube_pip*_*.csv (72).
...
L=/workspace/le-wm/eval_results/pip.log
...
Q=$BUCKET/qgate
log "start: PiP chain (4 arms, ALIEN gates reused)"
for round in $(seq 1 9000); do
  left=0
  for arm in pipbase pipq pipg05 pipg10; do
    run="lewm_${arm}_cube_s${SEED}"; exp="${arm}_cube"
    case $arm in
      pipbase) gate="" ;;
      pipq)    run="lewm_pipq_scale_cube_s${SEED}"; exp="pipq_scale_cube"; gate="" ;;
      pipg05)  run="lewm_pipg05_scale_cube_s${SEED}"; exp="pipg05_scale_cube"; gate="$Q/qgate_stage1_cube_alien_nce_lam0.05.json" ;;
      pipg10)  run="lewm_pipg10_scale_cube_s${SEED}"; exp="pipg10_scale_cube"; gate="$Q/qgate_stage1_cube_alien_nce_lam0.1.json" ;;
    esac
    if ! gcloud storage ls "$BUCKET/ckpts/$run/weights_epoch_10.pt" >/dev/null 2>&1; then
      left=1
      if [ -z "$gate" ]; then
        try "tr_${arm}" bash scripts/ray_train_qnative.sh cube_pip experiment="$exp" seed=$SEED
      else
        try "tr_${arm}" env QGATE_GCS="$gate" bash scripts/ray_train_qgate2.sh cube_pip experiment="$exp" seed=$SEED
      fi
      continue
    fi
    for sol in cem icem mppi; do
      for seeds in "101 102 103" "104 105 106"; do
        ...
        try "ev_${arm}_${sol}_${seeds%% *}" env PIP_OVERLAY=1 bash scripts/ray_eval_final.sh cube_pip "$arm" "$run" "$sol" $seeds
      done
    done
  done
  [ "$left" = 0 ] && { log "PIP COMPLETE (4 arms, 72 CSVs)"; exit 0; }
  sleep 240
done
