#!/usr/bin/env bash
# Full-q oracle on the frozen LeWM baseline for the remaining four tasks:
# fit the closed-form metric (task's complete non-velocity, non-leaking q), then
# evaluate cem x 6 seeds. nohup babysitter: GPU-aware, one submission per round,
...
L=/workspace/le-wm/eval_results/oracle_alltasks.log
...
SPECS=(
"pusht|ckpts|lewm_c1_s3072|pusht_c1_oracleF|canonical|c1oracleF"
"reacher|ckpts|lewm_r1_reacher_s3072|reacher_r1_oracleF|reacher_full|r1oracleF"
"tworoom|ckpts_tworoom|lewm_t1_tworoom_s3072|tworoom_t1_oracleF|canonical|t1oracleF"
"pointmaze|ckpts_pointmaze|lewm_p1_pointmaze_s3072|pointmaze_p1_oracleF|canonical|p1oracleF"
)
SEEDS="101,102,103,104,105,106"

log "start: full-q oracle on 4 tasks (fit + cem x 6 seeds)"
for round in $(seq 1 2000); do
  ...
  for spec in "${SPECS[@]}"; do
    ...
    if [ "$fit_done" = 0 ]; then
      left=1
      [ "$(nrun "ray_automet_oracle_any.sh $task")" != 0 ] && continue
      [ "$(free)" -lt 1 ] && continue
      n=${ATT[fit_$task]:-0}; [ "$n" -ge 4 ] && { log "fit_$task cap"; continue; }
      id=$(sub bash scripts/ray_automet_oracle_any.sh "$task" "$ckp" "$ckdir" "$tag" "$qvar")
  ...
  [ "$left" = 0 ] && { log "ALL FULL-Q ORACLES COMPLETE"; exit 0; }
  sleep 240
done
