...
python scripts/probe_mlp_q.py "$TASK" 2>&1 | tee "$SSD/probemlp_$TASK.log"

gcloud storage cp "eval_results/probe_mlp_$TASK.json" "$BUCKET/eval/"
gcloud storage cp "$SSD/probemlp_$TASK.log" "$BUCKET/eval/"
echo "PROBEMLP $TASK DONE"
