...
python scripts/probe_cumvar.py "$TASK" 2>&1 | tee "$SSD/cumvar_$TASK.log"

gcloud storage cp "eval_results/cumvar_$TASK.json" "eval_results/fig_cumvar_$TASK.png" "$BUCKET/eval/"
gcloud storage cp "$SSD/cumvar_$TASK.log" "$BUCKET/eval/"
echo "CUMVAR $TASK DONE"
