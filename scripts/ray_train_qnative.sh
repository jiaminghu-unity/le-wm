#!/usr/bin/env bash
# NATIVE-FULL q-only trainer (train_qnative.py registers extra variants): replica launcher + reacher.lance staging (the q-input
...
RUN=$(python train_qnative.py --cfg job --resolve "$@" 2>/dev/null \
...
python train_qnative.py "$@" 2>&1 | tee "$LOG"
