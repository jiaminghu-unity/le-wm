#!/usr/bin/env bash
# LpWM x SCALE cross-task extension (2026-09-07, user request): pusht + 4 multi-
# object OGBench tasks, 3 arms each (seed 3072), same recipe as the cube round:
#   lpwmbase_<t>  RDMReg+RepReLU only
#   lpwmq_<t>     + SCALE full native/config q (ungated)
#   lpwmg10_<t>   + gated by the task's existing lam-0.1 gate
...
L=/workspace/le-wm/eval_results/lpwm_multi.log
...
for round in $(seq 1 9000); do
  ...
  [ "$left" = 0 ] && { log "LPWM CROSS-TASK COMPLETE (15 arms, 198 CSVs)"; exit 0; }
  sleep 240
done
