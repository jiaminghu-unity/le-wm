#!/usr/bin/env bash
# Latent-geometry probe: which alternative planning costs CAN differ from the shipped
# squared L2, measured before committing GPU-days to a sweep.
#   usage: ray_latgeom.sh <pusht|reacher|cube>
#
# A COPY of ray_p5.sh (left untouched) with the analysis swapped and the render gate
# dropped -- this probe never renders. It runs only the model forward pass on dataset
# frames plus imagined rollouts, so there is no environment stepping and no pixels
# from the simulator; the fidelity gate would have nothing to gate.
# Results go to eval/latgeom_<task>.json; nothing existing is written.
set -euo pipefail
...
# invalidates them.
LOG="$SSD/latgeom_${TASK}.log"
: > "$LOG"

python scripts/probe_latent_geometry.py "$TASK" "${MODELS[@]}" "${EXTRA[@]}" 2>&1 | tee -a "$LOG"
for f in eval_results/latgeom_"$TASK"*.json; do [ -f "$f" ] && gcloud storage cp "$f" "$BUCKET/eval/" || true; done
gcloud storage cp "$LOG" "$BUCKET/eval/"
echo "LATGEOM DONE $TASK"
