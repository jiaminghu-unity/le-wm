...
case "$TASK" in
  pusht)   H5NAME=pusht_expert_train.h5; SRC="$BUCKET/datasets/pusht_expert_train.h5"; SUB=""
           MODELS=("q1:lewm_q1_qinput_s3072/weights_epoch_10.pt") ;;
...
python scripts/p4_qinput_pusht.py "${MODELS[@]}" "${EXTRA[@]}" 2>&1 | tee -a "$LOG"
gcloud storage cp "eval_results/p4_pusht_qinput.json" "$BUCKET/eval/" || true
: # no cache upload
