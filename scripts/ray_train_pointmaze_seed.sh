#!/usr/bin/env bash
# SEED-REPLICATION variant of ray_train_pointmaze.sh: hydra seed override (SEED env) threaded
...
RUN=$(python train_pointmaze.py --cfg job --resolve "experiment=$EXP" "seed=${SEED:?SEED env var required}" 2>/dev/null \
...
set +e
python train_pointmaze.py "experiment=$EXP" "seed=$SEED" 2>&1 | tee -a "$LOG"
rc=${PIPESTATUS[0]}
set -e
