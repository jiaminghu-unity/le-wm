#!/usr/bin/env bash
# SR sweep with the AutoMetric learned Mahalanobis planner cost. Derived from ray_eval_cost.sh:
# marked copy of ray_eval_final.sh); both are left untouched so they remain the
# provenance of the results they produced.
#   usage: METRIC=automet_cube_k1.pt ray_eval_automet.sh <task> <cfgname> <ckpt dir> <cem|icem> <seed>...
# Differences from ray_eval_l1.sh, all marked COST below:
#   * runs budget_sweep_automet.py --metric (fetched from $BUCKET/eval/), results to final_eval_automet/
#   * writes to final_eval_cost/ so neither final_eval/ nor final_eval_l1/ can be touched
#   * the FATAL gate gets the variant name, so a run that silently scored with another
#     variant cannot upload under this one's config label
set -euo pipefail

TASK="${1:?task}"; CFG="${2:?cfgname}"; CKPT_DIR="${3:?ckpt dir}"; SOLVER="${4:?solver}"; shift 4
SEEDS=("$@")
BUCKET=gs://prism-training-us/le-wm
METRIC="${METRIC:?GCS name of the automet_*.pt under eval/}"
OUTP="$BUCKET/final_eval_automet"
...
gcloud storage cp "$BUCKET/eval/$METRIC" "$SSD/$METRIC"
...
for S in "${SEEDS[@]}"; do
  ...
  OUT="final_${TASK}_${CFG}_automet_${SOLVER}_s${S}${EPS_N:+_$EPS_N}.csv"
  ...
  set +e
  # L1: the wrapper, same arguments. Output lands in a file first -- the numeric L1
  # proof prints before the sweep starts, and the `tail -40` the original uses to keep
  # job logs short would drop exactly that line.
  RUNLOG="$SSD/run_${OUT%.csv}.log"
  python scripts/budget_sweep_automet.py --metric "$SSD/$METRIC" \
    --env "$TASK" --solver "$SOLVER" \
    --config "$CFG" "$CKPT_DIR/weights_epoch_10.pt" \
    --tiers T1 T2 T3 T4 T5 \
    --episodes-json "$SSD/eps/$EPSNAME" \
    --out "$SSD/$OUT" > "$RUNLOG" 2>&1
  rc=$?; set -e
  # surface the proof in the job log too: it prints before the sweep starts, so the
  # tail below would never show it, and a verdict nobody can see is a verdict that
  # gets assumed rather than read
  grep "^\[cost\]" "$RUNLOG" || echo "[cost] (no verification line found)"
  tail -40 "$RUNLOG"
  # L1: FATAL, not a warning. Without this line the run scored with the shipped squared
  # cost, and the CSV would be L2 numbers filed under an l1 config name -- impossible
  # to tell apart afterwards.
  if ! grep -q "^\[automet\] criterion verified" "$RUNLOG"; then
    echo "FATAL: automet criterion not verified - refusing to upload $OUT" >&2
    gcloud storage cp "$RUNLOG" "$OUTP/logs/" || true
    exit 1
  fi
  [ "$rc" -ne 0 ] && RC=$rc
