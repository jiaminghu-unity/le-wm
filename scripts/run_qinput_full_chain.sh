#!/usr/bin/env bash
# NATIVE-FULL q-only trainings + their planning evals (2026-08-27, user: q-only
# 应该用原生完整 q;roster 换成 cube 22-d / tworoom 2-d / pointmaze 4-d native).
# Trainings first; each model's cem+icem evals start as soon as its own epoch-10
# ckpt lands. nohup babysitter conventions: GCS done-checks, dashboard-API free(),
# nrun dedup, one submission per round, attempt caps.
...
# task|cfg|run|launcher-args|solver|seeds  (launcher-args already includes the
# env argument where the launcher takes one, e.g. "ray_eval_qinput_any.sh cube")
EVALS=()
for spec in \
...
log "start: q-only v3 (trainings + evals; stale pusht eval block removed, eval scheduling ungated)"
for round in $(seq 1 4000); do
  ...
  for cell in "${EVALS[@]}"; do
    ...
    [ "$miss" = 0 ] && continue
    left=1
    # ckpt gate: this model's training not done yet
    gcloud storage ls "$BUCKET/ckpts/$run/weights_epoch_10.pt" >/dev/null 2>&1 || continue
  ...
  [ "$left" = 0 ] && { log "TRAININGS + EVALS ALL COMPLETE"; exit 0; }
  sleep 240
done
