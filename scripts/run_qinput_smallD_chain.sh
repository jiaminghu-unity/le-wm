#!/usr/bin/env bash
# Small-latent q-only ladder on Push-T (2026-08-23): D in {8, 32} vs the D=192
# canonical q-only model. Train, then eval cem + icem x 6 seeds through the
# q-input sweeper (state-based planning cost). nohup babysitter conventions.
...
L=/workspace/le-wm/eval_results/qinput_smallD.log
...
TRAINS=(
"d8||lewm_qinput_d8_s${SEED}|experiment=q_qinput_d8|bash scripts/ray_train_qteacher.sh pusht experiment=q_qinput_d8 data=pusht seed=${SEED}"
"d32||lewm_qinput_d32_s${SEED}|experiment=q_qinput_d32|bash scripts/ray_train_qteacher.sh pusht experiment=q_qinput_d32 data=pusht seed=${SEED}"
)
EVALS=()
for spec in "q1d8|lewm_qinput_d8_s${SEED}" "q1d32|lewm_qinput_d32_s${SEED}"; do
...
log "start: q-only small-D ladder (D=8, D=32; train -> cem/icem x 6)"
for round in $(seq 1 4000); do
  ...
  [ "$left" = 0 ] && { log "SMALL-D LADDER COMPLETE"; exit 0; }
  sleep 240
done
