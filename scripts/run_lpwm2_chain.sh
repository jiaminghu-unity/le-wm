#!/usr/bin/env bash
# LpWM v2 (official-recipe) campaign (2026-09-06): RDMReg + RepReLU under the unchanged LeWM
# architecture/recipe, cube, seed 3072. Arms:
#   lpwm2base  RDMReg only (LpWM baseline)
#   lpwm2q     + SCALE full 22-d config q (ungated)
#   lpwm2g10   + SCALE gated by pure-cube InfoNCE gate lam 0.1
# Evals cem/icem/mppi x 6 seeds -> final_eval/final_cube_lpwm*_*.csv (54).
...
L=/workspace/le-wm/eval_results/lpwm2.log
...
Q=$BUCKET/qgate
log "start: LpWM chain (3 arms)"
for round in $(seq 1 9000); do
  left=0
  for arm in lpwm2base lpwm2q lpwm2g10; do
    case $arm in
      lpwm2base) run="lewm_lpwm2base_cube_s${SEED}";      exp="lpwm2_base_cube";        gate="" ;;
      lpwm2q)    run="lewm_lpwm2q_scale_cube_s${SEED}";   exp="lpwm2_qfull_scale_cube"; gate="" ;;
      lpwm2g10)  run="lewm_lpwm2g10_scale_cube_s${SEED}"; exp="lpwm2_g10_scale_cube";   gate="$Q/qgate_stage1_cube_nce_lam0.1.json" ;;
    esac
    if ! gcloud storage ls "$BUCKET/ckpts/$run/weights_epoch_10.pt" >/dev/null 2>&1; then
      left=1
      if [ -z "$gate" ]; then
        try "tr_${arm}" bash scripts/ray_train_qnative.sh cube experiment="$exp" seed=$SEED
      else
        try "tr_${arm}" env QGATE_GCS="$gate" bash scripts/ray_train_qgate2.sh cube experiment="$exp" seed=$SEED
      fi
      continue
    fi
  ...
  [ "$left" = 0 ] && { log "LPWM2 COMPLETE (3 arms, 54 CSVs)"; exit 0; }
  sleep 240
done
