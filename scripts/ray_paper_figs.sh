#!/usr/bin/env bash
# Camera-ready probe figures (paper_figs.py): spectrum + top-k bars + full-q probe
# + distance-ratio stats. CPU-only. Writes only eval/fig_*_<task>.png and
# eval/paper_stats_<task>.json. Provisioning identical to ray_probe_pcq.sh.
set -euo pipefail
...
python scripts/paper_figs.py "$TASK" 2>&1 | tee "$SSD/paperfigs_$TASK.log"

gcloud storage cp "eval_results/paper_stats_$TASK.json" "eval_results/fig_spectrum_$TASK.png" \
  "eval_results/fig_topk_$TASK.png" "$BUCKET/eval/"
gcloud storage cp "$SSD/paperfigs_$TASK.log" "$BUCKET/eval/"
echo "PAPERFIGS $TASK DONE"
