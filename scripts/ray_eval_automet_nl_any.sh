#!/usr/bin/env bash
# AutoMetric/oracle metric SR evaluation, any task (cem; rank-only).
#   usage: ray_eval_automet_any.sh <task> <cfgname> <ckpt_prefix> <ckpt_dir> <metric.pt> <seed-list>
...
METRIC="${5:?automet_*.pt name under eval/}"; SEEDLIST="${6:?seed list}"
...
SWEEPER=scripts/budget_sweep_automet_nl.py
...
for S in "${SARR[@]}"; do
    ...
    OUT="final_${TASK}_${CFG}_automet_${SOLVER:-cem}_s${S}.csv"
    if gcloud storage ls "$OUTP/$OUT" >/dev/null 2>&1; then
      echo "[skip] $OUT"; continue
    fi
    echo "[run] $TASK $CFG automet cem seed=$S"
    set +e
    python "$SWEEPER" --metric "$SSD/$METRIC" \
      --env "$TASK" --solver cem \
