#!/usr/bin/env bash
# SR sweep with the planner cost switched from squared L2 to L1. A COPY of
# ray_eval_final.sh, which is left untouched, differing in exactly four lines
# (marked L1 below):
#   * runs budget_sweep_l1.py instead of budget_sweep.py
#   * writes to final_eval_l1/ instead of final_eval/, so no existing CSV can be
#     overwritten even by an identical (task, cfg, solver, seed)
#   * a FATAL gate on the numeric L1 proof budget_sweep_l1.py prints -- a wiring
#     mistake would otherwise file a full run of L2 numbers under an l1 name
#   * the usage text
# Checkpoints are read from ckpts/ (the unchanged full-q models) and never written.
# Episode sets, tiers, seeding and the render gate are the same code, which is what
# makes these numbers directly comparable to final_eval/ restricted to cem+icem.
#   usage: ray_eval_l1.sh <task> <cfgname> <ckpt dir under ckpts/> <cem|icem> <seed>...
set -euo pipefail

TASK="${1:?task}"; CFG="${2:?cfgname}"; CKPT_DIR="${3:?ckpt dir}"; SOLVER="${4:?solver}"; shift 4
SEEDS=("$@")
BUCKET=gs://prism-training-us/le-wm
OUTP="$BUCKET/final_eval_l1"        # L1: separate prefix
...
for S in "${SEEDS[@]}"; do
  ...
  python scripts/budget_sweep_l1.py \
  ...
  if ! grep -q "^\[cost\] L1 verified" "$RUNLOG"; then
    echo "FATAL: planner cost was not verified as L1 - refusing to upload $OUT" >&2
