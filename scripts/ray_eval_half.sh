#!/usr/bin/env bash
# SR sweep for the reduced-q ablation. A COPY of ray_eval_final.sh, which is left
# untouched, differing in exactly three lines (marked HALF: below):
#   * checkpoints are read from ckpts_half/ instead of ckpts/
#   * results are written to final_eval_half/ instead of final_eval/, so the existing
#     final_eval/ CSVs cannot be overwritten even by an identical (task,cfg,solver,seed)
#   * the usage text names the new prefix
# Everything else -- episode sets, tiers, solvers, the render gate, the skip-if-present
# logic -- is identical, which is what makes the reduced-q SR directly comparable to
# the full-q numbers already in final_eval/.
#   usage: ray_eval_half.sh <task> <cfgname> <ckpt dir under ckpts_half/> <solver> <seed>...
set -euo pipefail

TASK="${1:?task}"; CFG="${2:?cfgname}"; CKPT_DIR="${3:?ckpt dir}"; SOLVER="${4:?solver}"; shift 4
SEEDS=("$@")
BUCKET=gs://prism-training-us/le-wm
OUTP="$BUCKET/final_eval_half"          # HALF: separate prefix
...
# ---- checkpoint ----
gcloud storage cp "$BUCKET/ckpts_half/$CKPT_DIR/weights_epoch_10.pt" "$STABLEWM_HOME/checkpoints/$CKPT_DIR/"   # HALF
gcloud storage cp "$BUCKET/ckpts_half/$CKPT_DIR/config.json" "$STABLEWM_HOME/checkpoints/$CKPT_DIR/" || true    # HALF
