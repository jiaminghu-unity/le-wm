#!/usr/bin/env bash
# aux21 (full-config-q) cube training launcher: ray_train_replica.sh with the
# train_cube_full.py entry point (in-process q-variant registry merge).
#   usage: ray_train_cube_full.sh cube experiment=aux21_cube seed=3072
...
RUN=$(python train_cube_full.py --cfg job --resolve "$@" 2>/dev/null \
...
python train_cube_full.py "$@" 2>&1 | tee "$LOG"
