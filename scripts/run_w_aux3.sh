#!/usr/bin/env bash
# AutoMetric W on AUX-trained representations (qhead arms) for pusht/reacher/
# pointmaze -- does a q-aware representation change what W can learn? (cube k4
# already ran: +2.07, p=0.062). Train W, then cem x 6 seeds. nohup babysitter: GPU-aware, one submission per round,
...
L=/workspace/le-wm/eval_results/w_aux3.log
...
SPECS=(
"pusht|ckpts|lewm_c5_qhead0.3_s3072|pusht_c5_W|-|c5W"
"reacher|ckpts|lewm_r5_qhead0.4_s3072|reacher_r5_W|-|r5W"
"pointmaze|ckpts_pointmaze|lewm_p5_pointmaze_s3072|pointmaze_p5_W|-|p5W"
)
SEEDS="101,102,103,104,105,106"

log "start: W on Aux representations, 3 tasks (train + cem x 6 seeds)"
for round in $(seq 1 2000); do
  ...
  for spec in "${SPECS[@]}"; do
    ...
    if [ "$fit_done" = 0 ]; then
      left=1
      [ "$(nrun "ray_automet_train_any.sh $task")" != 0 ] && continue
      [ "$(free)" -lt 1 ] && continue
      n=${ATT[fit_$task]:-0}; [ "$n" -ge 4 ] && { log "fit_$task cap"; continue; }
      id=$(sub bash scripts/ray_automet_train_any.sh "$task" "$ckp" "$ckdir" "$tag")
  ...
  [ "$left" = 0 ] && { log "AUX-W RUNS COMPLETE"; exit 0; }
  sleep 240
done
