#!/usr/bin/env bash
# PIXEL-model eval for the self-collected OGBench multi-object tasks (stock budget_sweep).
#   usage: ray_eval_ogbmulti.sh <task> <cfgname> <ckpt_dir> <solver> <seed> [seed...]
...
for S in "${SEEDS[@]}"; do
  ...
  set +e
  python scripts/budget_sweep_ogbmulti.py \
