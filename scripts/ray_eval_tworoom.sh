...
#   * runs budget_sweep_tworoom.py, which registers the two-room ENV_PRESETS entry and then
...
# space guard: purge other staged datasets when low (all re-fetchable)
if [ ! -f "$DS/$H5NAME" ]; then
  ...
  if [ "${FREE_G:-0}" -lt 130 ]; then
    ...
    find "$STABLEWM_HOME/datasets" -mindepth 1 -maxdepth 2 ! -name "$H5NAME" -print -exec rm -rf {} + 2>/dev/null || true
...
# ---- checkpoint ----
gcloud storage cp "$BUCKET/ckpts_tworoom/$CKPT_DIR/weights_epoch_10.pt" "$STABLEWM_HOME/checkpoints/$CKPT_DIR/"
gcloud storage cp "$BUCKET/ckpts_tworoom/$CKPT_DIR/config.json" "$STABLEWM_HOME/checkpoints/$CKPT_DIR/" || true
