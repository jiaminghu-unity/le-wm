#!/usr/bin/env bash
# LpWM campaign (2026-09-06): RDMReg + RepReLU under the unchanged LeWM
# architecture/recipe, cube, seed 3072. Arms:
#   lpwmbase  RDMReg only (LpWM baseline)
#   lpwmq     + SCALE full 22-d config q (ungated)
#   lpwmg10   + SCALE gated by pure-cube InfoNCE gate lam 0.1
...
L=/workspace/le-wm/eval_results/lpwm.log
...
for round in $(seq 1 9000); do
  left=0
  for arm in lpwmbase lpwmq lpwmg10; do
    case $arm in
      lpwmbase) run="lewm_lpwmbase_cube_s${SEED}";      exp="lpwm_base_cube";        gate="" ;;
      lpwmq)    run="lewm_lpwmq_scale_cube_s${SEED}";   exp="lpwm_qfull_scale_cube"; gate="" ;;
      lpwmg10)  run="lewm_lpwmg10_scale_cube_s${SEED}"; exp="lpwm_g10_scale_cube";   gate="$Q/qgate_stage1_cube_nce_lam0.1.json" ;;
    esac
  ...
  [ "$left" = 0 ] && { log "LPWM COMPLETE (3 arms, 54 CSVs)"; exit 0; }
  sleep 240
done
