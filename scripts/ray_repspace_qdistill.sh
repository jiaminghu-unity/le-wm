#!/usr/bin/env bash
# Representation-space analysis for the metric-distilled SCALE (Push-T):
# spectrum / top-k PCA->q / R2 / rho_q vs rho_teacher, for
# {baseline, raw-q SCALE, Aux, distilled} + the q-only teacher itself.
# Dataset-only probe (lance frames), no env rendering needed.
set -euo pipefail
BUCKET=gs://prism-training-us/le-wm

SSD=/mnt/disks/ssd0
if ! mountpoint -q "$SSD"; then
  dev=$(lsblk -dnpo NAME,TYPE | awk '$2=="disk" && $1 ~ /nvme/ {print $1; exit}')
  [ -n "$dev" ] || { echo "FATAL: no local NVMe" >&2; exit 1; }
  sudo mkfs.ext4 -F -q -m 0 -E lazy_itable_init=0,lazy_journal_init=0,discard "$dev"
  sudo mkdir -p "$SSD" && sudo mount -o discard,defaults "$dev" "$SSD"
  sudo chmod a+w "$SSD"
fi
export STABLEWM_HOME="$SSD/stable-wm"
DS="$STABLEWM_HOME/datasets"; mkdir -p "$DS"

sudo apt-get update -q
sudo apt-get install -y -q swig build-essential zstd libgl1 libglib2.0-0 libxcb1 \
  libsm6 libxext6 libxrender1
if ! command -v uv >/dev/null; then
  pip install -q uv
  PATH="$(python3 -m site --user-base)/bin:$(python3 -c 'import sysconfig;print(sysconfig.get_path("scripts"))'):$PATH"
  export PATH; hash -r
fi
if [ ! -x "$SSD/.venv/bin/python" ]; then uv venv --python=3.10 "$SSD/.venv"; fi
source "$SSD/.venv/bin/activate"
uv pip install -q 'stable-worldmodel[train,env,format]'
uv pip install -q 'torch==2.12.1+cu126' torchvision --index-url https://download.pytorch.org/whl/cu126
uv pip install -q hdf5plugin -U datasets scikit-learn

[ -d "$DS/pusht_expert_train.lance" ] || \
  gcloud storage rsync -r "$BUCKET/datasets/pusht_expert_train.lance" "$DS/pusht_expert_train.lance"

for d in lewm_c1_s3072 lewm_c3_sig_obj0.1_s3072 lewm_c5_qhead0.3_s3072 \
         lewm_c10_qdistill0.1_s3072 lewm_q1_qinput_s3072; do
  mkdir -p "$STABLEWM_HOME/checkpoints/$d"
  gcloud storage cp "$BUCKET/ckpts/$d/weights_epoch_10.pt" "$STABLEWM_HOME/checkpoints/$d/"
  gcloud storage cp "$BUCKET/ckpts/$d/config.json" "$STABLEWM_HOME/checkpoints/$d/" || true
done

LOG="$SSD/repspace_qdistill_pusht.log"
python scripts/repspace_qdistill_pusht.py \
  "base:lewm_c1_s3072/weights_epoch_10.pt" \
  "scale:lewm_c3_sig_obj0.1_s3072/weights_epoch_10.pt" \
  "aux:lewm_c5_qhead0.3_s3072/weights_epoch_10.pt" \
  "distill:lewm_c10_qdistill0.1_s3072/weights_epoch_10.pt" \
  --teacher lewm_q1_qinput_s3072/weights_epoch_10.pt 2>&1 | tee "$LOG"
gcloud storage cp eval_results/repspace_qdistill_pusht.json "$BUCKET/eval/"
gcloud storage cp "$LOG" "$BUCKET/eval/" || true
