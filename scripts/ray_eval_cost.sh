#!/usr/bin/env bash
# SR sweep with the planner cost selected by $COST. A COPY of ray_eval_l1.sh (itself a
# marked copy of ray_eval_final.sh); both are left untouched so they remain the
# provenance of the results they produced.
#   usage: COST=dot ray_eval_cost.sh <task> <cfgname> <ckpt dir under ckpts/> <cem|icem> <seed>...
# Differences from ray_eval_l1.sh, all marked COST below:
#   * runs budget_sweep_cost.py --cost "$COST"
...
COST="${COST:?set COST to one of l2 l1 dot cos}"   # COST
OUTP="$BUCKET/final_eval_cost"   # COST: separate prefix
...
for S in "${SEEDS[@]}"; do
  ...
  python scripts/budget_sweep_cost.py --cost "$COST" \
  ...
  if ! grep -q "^\[cost\] $COST verified" "$RUNLOG"; then
    echo "FATAL: planner cost was not verified as $COST - refusing to upload $OUT" >&2
